...
//creating a global variable to store pizzas;
var movingPizzas = [];
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    var changes = [];
    //Changed the function so that we don't force composition, I did this by breaking the one loop into two. One
    //for calculating, the other for style adjustments.
    for (var i = 0; i < pizzaContainer.length; i++) {
      var dx = determineDx(pizzaContainer[i], size);
      changes.push((pizzaContainer[i].offsetWidth + dx) + 'px');
    }
    for (var i = 0; i < changes.length; i++) {
      pizzaContainer[i].style.width = changes[i];
    }
    //changes end here
  }
...
function updatePositions() {
  ...
  //replacing select all with select by classname
  //var items = document.querySelectorAll('.mover');
  //created a global variable so that this search wasn't needed every time.
  //var items = document.getElementsByClassName('mover');
  var movements = [];

  //making all phase calculations in one loop, then style adjustments in the next.
  for (var i = 0; i < movingPizzas.length; i++) {
    movements.push(Math.sin((document.body.scrollTop / 1250) + (i % 5)));
  }
  //Once I have the styles from the loop above, I can then make all style adjustments in another loop.
  for (var i = 0; i < movingPizzas.length; i++) {
    movingPizzas[i].style.left = movingPizzas[i].basicLeft + 100 * movements[i] + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //I chopped the # down to 50 from 200, since the most I every saw on a page was around 30. I picked 50 to be safe.
  for (var i = 0; i < 50; i++) {
  ...
  //I get the movingPizzas once, after they are created in the loop above. After that I don't have to keep finding them
  //inside the loop.
